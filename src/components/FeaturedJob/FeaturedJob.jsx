...
const FeaturedJob = () => {
  ...
  return (
    <div className='mt-12 relative w-3/4 mx-auto'>
      ...
      <div className='grid grid-rows lg:grid-cols-2 gap-4 my-4 w-75% justify-around'>
        {
          featuredJobs.slice(0,allJobs ? 6 : 4).map(featuredJob =><SingleJob key={featuredJob.id } featuredJob={featuredJob}></SingleJob>)
        }

      </div>
     <p onClick={handleAllJobs} ><button  className='button-see sm:mx-auto rounded bg-purple-700  text-center py-3 my-12 text-white w-32'>See More</button></p>
    </div>
